...
class WorkLoadController extends ApiActiveController
{
    ...
    public $table_name = 'work_load';
    public $controller_name = 'WorkLoad';
...
//        return $this->response(0, _e('There is an error occurred while processing.'), null, null, ResponseStatus::NOT_FOUND);
        ...
        $result = WorkLoad::createItem($model, $post);
        if (!is_array($result)) {
    ...
    {
        return $this->response(0, _e('There is an error occurred while processing.'), null, null, ResponseStatus::NOT_FOUND);
        $model = WorkLoad::findOne($id);
        if (!$model) {
            return $this->response(0, _e('Data not found.'), null, null, ResponseStatus::NOT_FOUND);
        }
        $post = Yii::$app->request->post();
        $this->load($model, $post);
        $result = WorkLoad::updateItem($model, $post);
        if (!is_array($result)) {
